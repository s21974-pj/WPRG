	...
	<?php
		if($_SERVER["REQUEST_METHOD"] == "POST"){
			if($_POST['quantity'] > 1 && !(isset($_POST['submit2']))){
				...
				echo('<form method="POST" action="zad2b-3.php">');
				...
				for($i = 2; $i <= $_POST['quantity']; $i++){					
					...
					echo('<input type = "text" id = "name'.$i.'" name = "name'.$i.'" required></div>');
				
					echo('<div><label for = "surname'.$i.'">Surname</label>');
					echo('<input type = "text" id = "surname'.$i.'" name = "surname'.$i.'" required></div>');
					
					echo('<div><label for = "email'.$i.'">Email</label>');
					echo('<input type = "email" id = "email'.$i.'" name = "email'.$i.'" required></div>');
